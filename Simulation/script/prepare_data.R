...
#                                                #
  ...
  # signature prevalence {0.2, 0.3, 0.4, 0.5(default), 0.6, 0.7, 0.8}
  abd.pt <- as.numeric(args[4])
  # signature effect size {0.5, 1, 1.5, 2(default), 2.5, 3}
  effect.sz <- as.numeric(args[5])
  # case/control sequence depth unevenness {1(default), 1.25 1.5, 1.75, 2}
  ...
  loc.vec <- c(Ka == 40, pos.pt == 0.7, abd.pt == 0.5, effect.sz == 2, mu == 1)
  if(sum(loc.vec) < 4){
    stop("Please only change one factor at the same time.\n")
  }else if(sum(loc.vec) == 5){
    data.loc <- c(paste0("./Simulation/By_taxnum/d", as.character(Ka), "/"),
                  paste0("./Simulation/By_pos_prop/p", as.character(pos.pt * 100), "/"),
                  paste0("./Simulation/By_abd_prop/p", as.character(abd.pt * 100), "/"),
                  paste0("./Simulation/By_effsz/sig", as.character(effect.sz + 1), "/"),
                  paste0("./Simulation/By_seqdepth/seq", as.character(mu), "/"))
  }else{
    if(which(!loc.vec) == 1){
      data.loc <-  paste0("./Simulation/By_taxnum/d", as.character(Ka), "/")
    }else if(which(!loc.vec) == 2){
      data.loc <-  paste0("./Simulation/By_pos_prop/p", as.character(pos.pt * 100), "/")
    }else if(which(!loc.vec) == 3){
      data.loc <-  paste0("./Simulation/By_abd_prop/p", as.character(abd.pt * 100), "/")
    }else if(which(!loc.vec) == 4){
      data.loc <-  paste0("./Simulation/By_effsz/sig", as.character(effect.sz + 1), "/")
    }else if(which(!loc.vec) == 5){
      data.loc <-  paste0("./Simulation/By_seqdepth/seq", as.character(mu), "/")
    }
  }
  for(loc in data.loc){
