...
# =============================================== #
  ...
  for(s in 1:100){
    load(paste0(data.loc, "Models_original/Melody.pool.org.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
  ...
  # MMUPHin batch correctet ----
  for(s in 1:100){
    load(paste0(data.loc, "Models_MMUPHin_batch_corrected/Melody.pool.mmu.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    ...
    AUPRC <- c(AUPRC, hg.pr$AUC)
    S <- c(S, s)
    linetype <- c(linetype, "MMUPHin")
    method <- c(method, "Melody2")
  }
  
  for(s in 1:100){
    load(paste0(data.loc, "Models_MMUPHin_batch_corrected/ANCOMBC.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    rank.fdr <- rank(ANCOMBC.model$res$p_val$labels1)
    true.active <- rep(0, K)
    names(true.active) <- tax.names
  ...
  for(s in 1:100){
    load(paste0(data.loc, "Models_MMUPHin_batch_corrected/Aldex2.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    true.active <- rep(0, K)
    names(true.active) <- tax.names
    true.active[signal.idx] <- 1
  ...
  for(s in 1:100){
    load(paste0(data.loc, "Models_MMUPHin_batch_corrected/BW.prop.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    rank.fdr <- rank(BW.prop.model$p.val)
    true.active <- rep(0, K)
    names(true.active) <- tax.names
  ...
  for(s in 1:100){
    load(paste0(data.loc, "Models_MMUPHin_batch_corrected/CLR.lasso.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    true.active <- rep(0, K)
    names(true.active) <- tax.names
    true.active[signal.idx] <- 1
  ...
  # COnQuR batch correctet ----
  for(s in 1:100){
    load(paste0(data.loc, "Models_ConQuR_batch_corrected/Melody.pool.con.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    ...
    AUPRC <- c(AUPRC, hg.pr$AUC)
    S <- c(S, s)
    linetype <- c(linetype, "ConQuR")
    method <- c(method, "Melody2")
  }
  
  for(s in 1:100){
    load(paste0(data.loc, "Models_ConQuR_batch_corrected/ANCOMBC.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    rank.fdr <- rank(ANCOMBC.model$res$p_val$labels1)
    true.active <- rep(0, K)
    names(true.active) <- tax.names
  ...
  for(s in 1:100){
    load(paste0(data.loc, "Models_ConQuR_batch_corrected/Aldex2.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    true.active <- rep(0, K)
    names(true.active) <- tax.names
    true.active[signal.idx] <- 1
  ...
  for(s in 1:100){
    load(paste0(data.loc, "Models_ConQuR_batch_corrected/BW.prop.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    rank.fdr <- rank(BW.prop.model$p.val)
    true.active <- rep(0, K)
    names(true.active) <- tax.names
  ...
  for(s in 1:100){
    load(paste0(data.loc, "Models_ConQuR_batch_corrected/CLR.lasso.model.",as.character(s),".Rdata"))
    load(paste0(data.loc, "signal/signal.",as.character(s),".Rdata"))
    true.active <- rep(0, K)
    names(true.active) <- tax.names
    true.active[signal.idx] <- 1
