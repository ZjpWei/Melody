...
#                                                #
  ...
  # Load data
  load("./Prediction/prepare_data/data.rel.batch.all.Rdata")
  ...
  control.id.lst <- list()
  for(l in 1:L){
    case.id.lst[[l]] <- sample(which(data.rel.batch[[l]]$X == 1))
    control.id.lst[[l]] <- sample(which(data.rel.batch[[l]]$X == 0))
  }
