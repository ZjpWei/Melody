...
Get_lasso_pre = function(Melody){
  ...
  if(L == 1){
    ...
    taxa.mat <- matrix(c(taxa.set[[1]], FALSE)[order(idx)], nrow = 1)
  }else{
    for(l in 1:L){
      idx = c(setdiff(1:K, ref[l]), ref[l])
      taxa.mat <- rbind(taxa.mat, c(taxa.set[[l]], FALSE)[order(idx)])
    }
  }
...
byabess_AA <- function(Melody, delta, support.size){
  ### Load data
  summary.stat.study <- Melody$summary.stat.study
  taxa.set <- Melody$taxa.set
  ...
  if(length(delta) == 1){
    for(l in 1:L){
      ref.shift <- c(ref.shift, rep(delta, sum(taxa.set[[l]])))
    }
  }else{
    for(l in 1:L){
      ref.shift <- c(ref.shift, rep(delta[l], sum(taxa.set[[l]])))
    }
  }
  ...
  names(delta) <- Melody$dat.inf$ref
...
search.ref.loc <- function(Melody,
  ...
  if(is.null(result)){
    ...
    initial.result <- byabess_AA(Melody = Melody, delta = - ref.quant, support.size = lambda)
  ...
  while(loop.range){
    if(s.1.GIC < GIC.result){
      ...
      ref.quant <- cal.quantile(quantile_sm = quantile_sm, prop = s.1.prop)
      s.1.result <- byabess_AA(Melody = Melody, delta = - ref.quant, support.size = lambda)
    ...
    if(s.2.GIC < GIC.result){
      ...
      s.2.result <- byabess_AA(Melody = Melody, delta = - ref.quant, support.size = lambda)
  ...
  s.left.result <- byabess_AA(Melody = Melody, delta = - ref.quant, support.size = lambda)
  ...
  s.right.result <- byabess_AA(Melody = Melody, delta = - ref.quant, support.size = lambda)
  ...
  while(loop.study){
    ...
    if(s.all.GIC[1] == min(s.all.GIC) & abs(s.all.prop[1] - s.all.prop[2]) <= tol){
      ...
      s.new.result <- byabess_AA(Melody = Melody, delta = - ref.quant, support.size = lambda)
...
melody.get.test <- function(delta, Melody,
                            p.adjust.methods = c("fdr", "holm", "hochberg", "hommel",
  ...
  for(l in 1:L){
    ...
    mu_mat[l,names(beta_hat)] <- (beta_hat - delta[l]) / sigma2
...
search.test.delta <- function(Melody, L, quantile_sm, tmp.result, search.loc, GIC.result, initial.range, tol, verbose){
  loop <- TRUE
  loop.time <- 0
  # if(verbose){
  #   message(paste0("-- searching model with subset size:", s.size, " --"))
  ...
  while(loop){
    ...
    GIC.result.tmp <- GIC.result
    for(l in 1:L){
      tmp.search <- search.delta.loc(Melody = Melody,
                                     study.l = l,
                                     quantile_sm = quantile_sm,
                                     search.prop = search.loc.tmp,
                                     result = tmp.result,
...
### Search delta
search.delta.loc <- function(Melody,
                             study.l,
                             quantile_sm,
                             search.prop,
                             result,
  ...
  if(is.null(result)){
    ...
    s.right.prop <- rep(1/2, L)
    ref.quant <- cal.quantile(quantile_sm = quantile_sm, prop = s.1.prop)
    initial.result <- byglm(Melody = Melody, delta = - ref.quant)
    GIC.result <- initial.result$q_loss
  }else{
    s.1.prop <- search.prop
    s.2.prop <- search.prop
    s.left.prop <- search.prop
    s.right.prop <- search.prop
    GIC.result <- result$q_loss
  }
  loop.range <- TRUE
  s.1.GIC <- -Inf
  s.2.GIC <- -Inf
  while(loop.range){
    if(s.1.GIC < GIC.result){
      s.1.prop[study.l] <- max(s.1.prop[study.l] - initial.range / 2, 0)
      ref.quant <- cal.quantile(quantile_sm = quantile_sm, prop = s.1.prop)
      s.1.result <- byglm(Melody = Melody, delta = - ref.quant)
      s.1.GIC <- s.1.result$q_loss
    }
    if(s.2.GIC < GIC.result){
      s.2.prop[study.l] <- min(s.2.prop[study.l] + initial.range / 2, 1)
      ref.quant <- cal.quantile(quantile_sm = quantile_sm, prop = s.2.prop)
      s.2.result <- byglm(Melody = Melody, delta = - ref.quant)
      s.2.GIC <- s.2.result$q_loss
    }
    if(s.1.GIC > GIC.result & s.2.GIC > GIC.result){
      loop.range <- FALSE
    }else if(s.1.prop[study.l] == 0 | s.2.prop[study.l] == 1){
  ...
  s.left.prop[study.l] <- s.1.prop[study.l] * p.pi + s.2.prop[study.l] * (1 - p.pi)
  ref.quant <- cal.quantile(quantile_sm = quantile_sm, prop = s.left.prop)
  s.left.result <- byglm(Melody = Melody, delta = - ref.quant)
  s.left.GIC <- s.left.result$q_loss
  #####
  s.right.prop[study.l] <- s.1.prop[study.l] * (1 - p.pi) + s.2.prop[study.l] * p.pi
  ref.quant <- cal.quantile(quantile_sm = quantile_sm, prop = s.right.prop)
  s.right.result <- byglm(Melody = Melody, delta = - ref.quant)
  s.right.GIC <- s.right.result$q_loss
  #####
  # we have:
  # s.1.result; s.1.GIC
  # s.2.result; s.2.GIC
  ...
  while(loop.study){
    ...
    if(s.all.GIC[1] == min(s.all.GIC) & abs(s.all.prop[1] - s.all.prop[2]) <= tol){
      ...
      s.new.prop <- s.1.prop
      ref.quant <- cal.quantile(quantile_sm = quantile_sm, prop = s.new.prop)
      s.new.result <- byglm(Melody = Melody, delta = - ref.quant)
    }else if(s.all.GIC[4] == min(s.all.GIC) & abs(s.all.prop[4] - s.all.prop[3]) <= tol){
      loop.study <- FALSE
      s.new.prop <- s.2.prop
      ref.quant <- cal.quantile(quantile_sm = quantile_sm, prop = s.new.prop)
  ...
  return(list(study.min = s.new.prop, study.min.GIC = s.new.result$q_loss, result = s.new.result))
}

### Optimization function
byglm <- function(Melody, delta){
  ### Load data
  summary.stat.study <- Melody$summary.stat.study
  taxa.set <- Melody$taxa.set
  X.enlarge <- Melody$lasso.mat$X.enlarge
  ...
  ### Abess model
  result <- glm.fit(x = X.enlarge,
                    y = Y.enlarge,
                    family = gaussian(),
                    intercept = FALSE)
  
  fit.coef <- result$coefficients
  q_loss <- sum( (Y.enlarge - X.enlarge %*% result$coefficients)^2 )
  mu.fit <- fit.coef[1:mu.len]
  mu.fit <- c(mu.fit, rep(0, length(empty.id)))[order(c(nonempty.id ,empty.id))]
  names(mu.fit) <- Melody$dat.inf$taxa.names
  names(delta) <- Melody$dat.inf$ref
