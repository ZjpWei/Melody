...
# =============================================== #
  ...
  tax.inters <- c()
  for(ss in 1:5){
    load(paste0("./CRC/sensitivity/Melody.model.ref",as.character(ss),".Rdata"))
    if(ss == 1){
    ...
    tax.inters[[ss]] <- names(which(Melody.model$disease$coef!=0))
  }
  ...
  for(ss in 1:5){
    load(paste0("./CRC/sensitivity/Melody.model.ref",as.character(ss),".Rdata"))
    load(paste0("./CRC/Sensitivity/summary.stats.ref",as.character(ss),".Rdata"))
    if(ss == 1){
    ...
    for(l in 1:L){
      ...
      tmp.RA[names(summary.stat.study[[l]]$est[,1])] <- summary.stat.study[[l]]$est[,1]
      tmp.AA <- tmp.RA + Melody.model$disease$delta[l]
      tmp.AA[names(summary.stat.study[[l]]$est[,1])] <- tmp.AA[names(summary.stat.study[[l]]$est[,1])] 
      tmp.RA[names(summary.stat.study[[l]]$est[,1])] <- tmp.RA[names(summary.stat.study[[l]]$est[,1])]
  ...
  # Generate figures ----
  pdf("./figures/FigS7_RA.pdf",         # File name
      width = 7.67, height = 3.78, # Width and height in inches
      bg = "white")


  plot_grid(RA.lst[[1]], RA.lst[[2]], RA.lst[[3]],
            RA.lst[[4]], RA.lst[[5]],
            nrow=5, align = 'v', rel_heights = rep(1,5)/5)

  dev.off()

  pdf("./figures/FigS7_AA.pdf",         # File name
      width = 7.67, height = 3.78, # Width and height in inches
      bg = "white")

  plot_grid(AA.lst[[1]], AA.lst[[2]], AA.lst[[3]],
            AA.lst[[4]], AA.lst[[5]],
            nrow=5, align = 'v', rel_heights = rep(1,5)/5)
