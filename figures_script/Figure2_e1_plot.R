...
#                                                #
  ...
  rm(list = ls())
  # General
  data.all.loc <- "./CRC_Real/CRC_all_K401/"
  data.loso.loc <- "./CRC_Real/CRC_loso_K401/"
  ...
  methods <- c()
  study <- c()
  load(paste0(data.all.loc, "Models/Melody.model.all.Rdata"))
  global.meta <- list(names(which(Melody.model$coef!=0)))
  meta.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models/Melody.model.", as.character(l),".Rdata"))
  ...
  ############################### ANCOM-BC ##################################
  load(paste0(data.all.loc, "Models/ANCOMBC.model.all.Rdata"))
  global <- list(which(ANCOMBC.model$res$q_val$labels1 <= fdr))
  ANCOMBC.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models/ANCOMBC.model.", as.character(l),".Rdata"))
  ...
  ############################### ALDEx2 ####################################
  load(paste0(data.all.loc, "Models/Aldex2.model.all.Rdata"))
  Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
  global <- list(which(Aldex2.fdr <= fdr))
  Aldex2.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models/Aldex2.model.", as.character(l), ".Rdata"))
    Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
  ...
  ############################### BW-prop ###################################
  load(paste0(data.all.loc, "Models/BW.prop.model.all.Rdata"))
  global <- list(which(BW.prop.model$q.val <= fdr))
  BW.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models/BW.prop.model.", as.character(l),".Rdata"))
  ...
  ############################### CLR-LASSO #################################
  load(paste0(data.all.loc, "Models/CLR.lasso.model.all.Rdata"))
  global.clrlasso <- list(names(which(clrlasso$glmnet.fit$beta[,clrlasso$lambda == clrlasso$lambda.min]!=0)))
  clrlasso.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models/CLR.lasso.model.", as.character(l), ".Rdata"))
  ...
  methods <- c()
  study <- c()
  load(paste0(data.all.loc, "Models_batch/ANCOMBC.model.all.Rdata"))
  global <- list(which(ANCOMBC.model$res$q_val$labels1 <= fdr))
  ANCOMBC.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models_batch/ANCOMBC.model.", as.character(l),".Rdata"))
    ANCOMBC.rk[[l]] <- which(ANCOMBC.model$res$q_val$labels1 <= fdr)
  }
  ANCOMBC.pr <- list()
  for(l in 1:5){
  ...
  ############################### ALDEx2 ###################################
  load(paste0(data.all.loc, "Models_batch/Aldex2.model.all.Rdata"))
  Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
  global <- list(which(Aldex2.fdr <= fdr))
  Aldex2.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models_batch/Aldex2.model.", as.character(l), ".Rdata"))
    Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
    Aldex2.rk[[l]] <- which(Aldex2.fdr <= fdr)
  }
  Aldex2.pr <- list()
  ...
  ################################ BW-prop ##################################
  load(paste0(data.all.loc, "Models_batch/BW.prop.model.all.Rdata"))
  global <- list(which(BW.prop.model$q.val <= fdr))
  BW.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models_batch/BW.prop.model.", as.character(l),".Rdata"))
    BW.rk[[l]] <- which(BW.prop.model$q.val <= fdr)
  }
  BW.pr <- list()
  for(l in 1:5){
  ...
  ############################### CLR-LASSO #################################
  load(paste0(data.all.loc, "Models_batch/CLR.lasso.model.all.Rdata"))
  global.clrlasso <- list(names(which(clrlasso$glmnet.fit$beta[,clrlasso$lambda == clrlasso$lambda.min]!=0)))
  clrlasso.rk <- list()
  for(l in 1:5){
    load(paste0(data.loso.loc, "Models_batch/CLR.lasso.model.", as.character(l), ".Rdata"))
    clrlasso.rk[[l]] <- names(which(clrlasso$glmnet.fit$beta[,clrlasso$lambda == clrlasso$lambda.min]!=0))
  }
  clrlasso.pr <- list()
  for(l in 1:5){
