...
# =============================================== #
  ...
  PRC_all <- PRC_all[PRC_all$Method %in% c("Melody", "Melody-Pooled", "ALDEx2", "ANCOM-BC2", "MMUPHin","BW","CLR-LASSO"),]
  ...
  p.large <- PRC_all %>%
    ...
    xlab("Number of signatures") + ylim(0.38,0.83) + ggtitle("Sample size 100-180") +
  ...
  p.small <- PRC_all %>%
    ggplot(aes(x=x.label, y=AUPRC, group = interaction(Method, batch_type), linetype = batch_type))  +
    xlab("Number of signatures") + ylim(0.38,0.83) + ggtitle("Sample size 20-60") +
    geom_line(linewidth = 0.7, aes(color=Method)) + geom_point(size = 2,aes(color = Method), pch = 18) +
    scale_color_manual(
      breaks =  c("Melody", "Melody-Pooled", "MMUPHin","CLR-LASSO","BW", "ALDEx2", "ANCOM-BC2"),
      values = c("red", "pink", "purple", "orange", "grey", "green" ,"blue"))  +
  ...
  pdf("./Simulation_v2/FigS12.pdf", width = 13.83, height = 9, bg = "white")
