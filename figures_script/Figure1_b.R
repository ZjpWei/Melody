...
#                                                #
  ...
  facts <- c("sig2", "sig2.5", "sig3", "sig3.5", "sig4")
  ...
  PRC_all$x.label[PRC_all$x.label == "seq1"] <- "0"
  PRC_all$x.label[PRC_all$x.label == "seq1_25"] <- "0.25"
  PRC_all$x.label[PRC_all$x.label == "seq1_50"] <- "0.5"
  PRC_all$x.label[PRC_all$x.label == "seq1_75"] <- "0.75"
  PRC_all$x.label[PRC_all$x.label == "seq2"] <- "1"
  PRC_all$x.label <- factor(PRC_all$x.label, levels = unique(PRC_all$x.label), ordered = TRUE)
  PRC_all$method <- factor(PRC_all$method, levels = unique(PRC_all$method), ordered = TRUE)
  PRC_all <- PRC_all[PRC_all$method %in% c("Melody","ALDEx2","ANCOM-BC","BW", "CLR-LASSO"),]
  # Customizing the output
