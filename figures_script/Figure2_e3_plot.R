  ##############################################
#                                                #
# Figure 2 (e2) Stability: K401 v.s. K267(order) #
#                                                #
  ##############################################
  ...
  load("./CRC_Real/CRC_all_order/prepare_data/data.rel.all.Rdata")
  ...
  data.loc.1 <- "./CRC_Real/CRC_loso_K401/"
  data.loc.2 <- "./CRC_Real/CRC_loso_order/"
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models/Melody.model.all.Rdata")
      melody.id <- names(Melody.model$coef)
    }else{
      load(paste0(data.loc.2, "Models/Melody.model.", as.character(l),".Rdata"))
      melody.id <- names(Melody.model$coef)
    }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_K401/Models/Melody.model.all.Rdata")
      melody.id.2 <- names(Melody.model$coef)
      filters.id <- intersect(melody.id, melody.id.2)
    }else{
      load(paste0(data.loc.1, "Models/Melody.model.", as.character(l),".Rdata"))
      melody.id.2 <- names(Melody.model$coef)
      filters.id <- intersect(melody.id, melody.id.2)
    }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models/ANCOMBC.model.all.Rdata")
      ANCOMBC.rk.1[[k]] <- which(ANCOMBC.model$res$q_val$labels1 <= fdr)
    }else{
      load(paste0(data.loc.2, "Models/ANCOMBC.model.", as.character(l),".Rdata"))
      ANCOMBC.rk.1[[k]] <- which(ANCOMBC.model$res$q_val$labels1 <= fdr)
    }
    k <- k + 1
  }
  ...
  for(l in replicates){
    if(l == "all"){
      ...
      load(paste0(data.loc.1, "Models/ANCOMBC.model.", as.character(l),".Rdata"))
      ANCOMBC.rk.2[[k]] <- which(ANCOMBC.model$res$q_val$labels1[filters] <= fdr)
    }
    k <- k + 1
  }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models/Aldex2.model.all.Rdata")
      Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
      Aldex2.rk.1[[k]] <- which(Aldex2.fdr <= fdr)
    }else{
      load(paste0(data.loc.2, "Models/Aldex2.model.", as.character(l),".Rdata"))
      Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
      Aldex2.rk.1[[k]] <- which(Aldex2.fdr <= fdr)
    }
    k <- k + 1
  ...
  for(l in replicates){
    if(l == "all"){
      ...
      load(paste0(data.loc.1, "Models/Aldex2.model.", as.character(l),".Rdata"))
      Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
      Aldex2.rk.2[[k]] <- which(Aldex2.fdr[filters] <= fdr)
    }
    k <- k + 1
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models/BW.prop.model.all.Rdata")
    }else{
      load(paste0(data.loc.2, "Models/BW.prop.model.", as.character(l),".Rdata"))
    }
  ...
  for(l in replicates){
    if(l == "all"){
      ...
      load(paste0(data.loc.1, "Models/BW.prop.model.", as.character(l),".Rdata"))
    }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models/CLR.lasso.model.all.Rdata")
    }else{
      load(paste0(data.loc.2, "Models/CLR.lasso.model.", as.character(l), ".Rdata"))
    }
  ...
  for(l in replicates){
    if(l == "all"){
      ...
      load(paste0(data.loc.1,"Models/CLR.lasso.model.", as.character(l), ".Rdata"))
    }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models_batch/ANCOMBC.model.all.Rdata")
      ANCOMBC.rk.1[[k]] <- which(ANCOMBC.model$res$q_val$labels1 <= fdr)
    }else{
      load(paste0(data.loc.2, "Models_batch/ANCOMBC.model.", as.character(l),".Rdata"))
      ANCOMBC.rk.1[[k]] <- which(ANCOMBC.model$res$q_val$labels1 <= fdr)
    }
    k <- k + 1
  }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_K401/Models_batch/ANCOMBC.model.all.Rdata")
      ANCOMBC.rk.2[[k]] <- which(ANCOMBC.model$res$q_val$labels1[filters] <= fdr)
    }else{
      load(paste0(data.loc.1, "Models_batch/ANCOMBC.model.", as.character(l),".Rdata"))
      ANCOMBC.rk.2[[k]] <- which(ANCOMBC.model$res$q_val$labels1[filters] <= fdr)
    }
    k <- k + 1
  }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models_batch/Aldex2.model.all.Rdata")
      Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
      Aldex2.rk.1[[k]] <- which(Aldex2.fdr <= fdr)
    }else{
      load(paste0(data.loc.2, "Models_batch/Aldex2.model.", as.character(l),".Rdata"))
      Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
      Aldex2.rk.1[[k]] <- which(Aldex2.fdr <= fdr)
    }
    k <- k + 1
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_K401/Models_batch/Aldex2.model.all.Rdata")
      Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
      Aldex2.rk.2[[k]] <- which((Aldex2.fdr)[filters] <= fdr)
    }else{
      load(paste0(data.loc.1, "Models_batch/Aldex2.model.", as.character(l),".Rdata"))
      Aldex2.fdr <- Aldex2.model$glmfit$`labels1:pval.BH`
      Aldex2.rk.2[[k]] <- which(Aldex2.fdr[filters] <= fdr)
    }
    k <- k + 1
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models_batch/BW.prop.model.all.Rdata")
    }else{
      load(paste0(data.loc.2, "Models_batch/BW.prop.model.", as.character(l),".Rdata"))
    }
    BW.rk.1[[k]] <- which(BW.prop.model$q.val <= fdr)
    k <- k + 1
  }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_K401/Models_batch/BW.prop.model.all.Rdata")
    }else{
      load(paste0(data.loc.1, "Models_batch/BW.prop.model.", as.character(l),".Rdata"))
    }
    BW.rk.2[[k]] <- which(BW.prop.model$q.val[filters] <= fdr)
    k <- k + 1
  }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_order/Models_batch/CLR.lasso.model.all.Rdata")
    }else{
      load(paste0(data.loc.2, "Models_batch/CLR.lasso.model.", as.character(l), ".Rdata"))
    }
    clrlasso.rk.1[[k]] <- names(which(clrlasso$glmnet.fit$beta[,clrlasso$lambda == clrlasso$lambda.min]!=0))
    k <- k + 1
  }
  ...
  for(l in replicates){
    if(l == "all"){
      load("./CRC_Real/CRC_all_K401/Models_batch/CLR.lasso.model.all.Rdata")
    }else{
      load(paste0(data.loc.1,"Models_batch/CLR.lasso.model.", as.character(l), ".Rdata"))
    }
    clrlasso.tmp <-  clrlasso$glmnet.fit$beta[filters,]
    clrlasso.rk.2[[k]] <- names(which(clrlasso.tmp[,clrlasso$lambda == clrlasso$lambda.min] != 0))
    k <- k + 1
