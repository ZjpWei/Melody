...
# =============================================== #
  ...
  plot.single.study.heatmap.ref <- function(x){
    ...
    g1 <- df.plot %>%
      ...
      scale_fill_gradientn(colours=col.hm, limits=c(-mx, mx),
                           ...
                           labels = c("-2", as.character(-1:1), "2")) +
      guides(fill = guide_colourbar(barwidth = 12, barheight = 0.6, title = "Coefficient",
                                    title.position = "top"))
  }
  ...
  pdf("./figures/figure4_a.pdf", width = 12, height = 6.2, bg = "white")
