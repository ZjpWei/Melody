...
#                                                #
  ...
  load("./CRC_Real/CRC_all_K401/summary.stats/summary.stats.ref1.Rdata")
  source("./CRC_Real/script/utility_meta.R")
  ...
  ################ change delta 1 ##############
  GIC.r1 <- NULL
  ref1.rg <- seq(from = quantile_sm[1,1], to = quantile_sm[1,2], length.out = 100)
  tmp.ref <- best_ref
  for(ref1 in ref1.rg){
    tmp.ref[1] <- ref1
  ...
  ################ change delta 2 ##############
  GIC.r2 <- NULL
  ref2.rg <- seq(from = quantile_sm[2,1], to = quantile_sm[2,2], length.out = 100)
  tmp.ref <- best_ref
  for(ref2 in ref2.rg){
    tmp.ref[2] <- ref2
    a <- byabess_AA(Melody = summary.stat.study.ref1, 
  ...
  ################ change delta 3 ##############
  GIC.r3 <- NULL
  ref3.rg <- seq(from = quantile_sm[3,1], to = quantile_sm[3,2], length.out = 300)
  tmp.ref <- best_ref
  for(ref3 in ref3.rg){
    tmp.ref[3] <- ref3
    a <- byabess_AA(Melody = summary.stat.study.ref1, 
  ...
  ################ change delta 4 ##############
  GIC.r4 <- NULL
  ref4.rg <- seq(from = quantile_sm[4,1], to = quantile_sm[4,2], length.out = 400)
  tmp.ref <- best_ref
  for(ref4 in ref4.rg){
    tmp.ref[4] <- ref4
    a <- byabess_AA(Melody = summary.stat.study.ref1, 
  ...
  ################ change delta 5 ##############
  GIC.r5 <- NULL
  ref5.rg <- seq(from = quantile_sm[5,1], to = quantile_sm[5,2], length.out = 500)
  tmp.ref <- best_ref
  for(ref5 in ref5.rg){
    tmp.ref[5] <- ref5
    a <- byabess_AA(Melody = summary.stat.study.ref1, 
  ...
  # Plotting
  p1 <- data.frame(s = 20:120, HBIC = GIC.s) %>% ggplot(aes(x = s, y = HBIC)) + 
    geom_line() + xlim(20,122) +
