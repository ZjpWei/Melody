...
# =============================================== #
  ...
  # Scenario: n = 100 ~ 180 ----
  data.loc <- "./Simulation/large/Longitudinal/"
  ...
  PRC_all$Method <- factor(PRC_all$method, levels = c("Melody", "MMUPHin", "ANCOM-BC2"), ordered = TRUE)
  PRC_all <- PRC_all[PRC_all$Method %in% c("Melody", "ANCOM-BC2", "MMUPHin"),]
  ...
  p.large <- PRC_all %>%
    ...
    geom_line(linewidth = 0.7, aes(color=Method), position = position_dodge(width = 0.7)) +
    geom_point(size = 2,aes(color = Method), position = position_dodge(width = 0.7), pch = 18) +
    scale_color_manual(breaks =  c("Melody", "MMUPHin", "ANCOM-BC2"), values = c("red","purple", "blue"))  +
    ...
    geom_errorbar(aes(ymin = lower, ymax = upper, color = Method),width = 0.1,
                  position = position_dodge(width = 0.7)) + theme_bw() +
  ...
  # Scenario: n = 20 ~ 60 ----
  data.loc <- "./Simulation/small/Longitudinal/"
  facts <- c("4", "20", "40", "80", "160")
  PRC_all <- NULL
  for(pos.lst in c(0.6, 0.8, 1)){
    for(u.lst in c(0, 0.5, 1)){
  ...
  p.small <- PRC_all %>%
    ggplot(aes(x=x.label, y=AUPRC, group = interaction(Method, batch_type), linetype = batch_type))  +
    xlab("Number of signatures") + ylab("AUPRC")  + ylim(0.2,1) + ggtitle("Sample size 20-60") +
    geom_line(linewidth = 0.7, aes(color=Method), position = position_dodge(width = 0.7)) +
    geom_point(size = 2,aes(color = Method), position = position_dodge(width = 0.7), pch = 18) +
    scale_color_manual(breaks =  c("Melody", "MMUPHin", "ANCOM-BC2"), values = c("red","purple", "blue"))  +
    scale_linetype_manual(breaks = c( "Original", "Study-effect corrected"), values=c("solid", "dashed")) +
  ...
  pdf("./figures/FigS3.pdf", width = 15, height = 9, bg = "white")
