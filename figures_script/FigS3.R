...
# =============================================== #
  ...
  # Scenario: n = 100 ~ 180 ----
  data.loc <- "./Simulation_v2/large/Longitudinal//"
  ...
  p.large <- PRC_all %>%
    ...
    xlab("Number of signatures") + ylab("AUPRC")  +  ylim(0.5,0.9) + ggtitle("Sample size 100-180") +
    geom_line(linewidth = 0.7, aes(color=Method)) + geom_point(size = 2,aes(color = Method), pch = 18) +
    scale_color_manual(
      breaks =  c("Melody", "MMUPHin", "ANCOM-BC2"),
  ...
  # Scenario: n = 20 ~ 60 ----
  data.loc <- "./Simulation_v2/small/Longitudinal/"
  facts <- c("20", "30", "40", "50", "60", "70", "80")
  ...
  p.small <- PRC_all %>%
    ggplot(aes(x=x.label, y=AUPRC, group = interaction(Method, batch_type), linetype = batch_type))  +
    xlab("Number of signatures") + ylab("AUPRC")  + ylim(0.4,0.9) + ggtitle("Sample size 20-60") +
    geom_line(linewidth = 0.7, aes(color=Method)) + geom_point(size = 2,aes(color = Method), pch = 18) +
    scale_color_manual(
      breaks =  c("Melody", "MMUPHin", "ANCOM-BC2"),
      values = c("red","purple", "blue"))  +
  ...
  pdf("./Simulation_v2/FigS3.pdf", width = 13.83, height = 9, bg = "white")
