...
#                                                #
  ...
  for(methodology in c("Melody", "clrlasso", "Aldex2", "ANCOM", "BW")){
    ...
    for(a in c(1:100))
    {
      load(paste0(data.loc, "AUROC_original/auroc.", as.character(a), ".prop.",methodology , ".Rdata"))
    ...
    if(methodology %in% c("clrlasso", "Aldex2", "ANCOM", "BW")){
      ...
      for(a in c(1:100))
      {
        load(paste0(data.loc, "AUROC_batch_corrected/auroc.", as.character(a), ".prop.",methodology , ".Rdata"))
        auc_tmp <- rbind(auc_tmp, auroc.all$AUC)
      }
      AUC <- c(AUC, colMeans(auc_tmp, na.rm = TRUE))
      taxa.num <- c(taxa.num, auroc.all$taxa.num)
  ...
  for(methodology in c("Melody", "clrlasso","Aldex2", "ANCOM", "BW")){
    ...
    if(methodology %in% c("clrlasso", "Aldex2", "ANCOM", "BW")){
      for(s in 1:5){
        ...
        auc_tmp <- NULL
        for(a in c(1:100)){
          loc <- paste0(data.loc, "AUROC_batch_corrected/auroc.", as.character(s), ".", as.character(a), ".prop.",methodology , ".Rdata")
          if(file.exists(loc)){
            load(loc)
            auc_tmp <- rbind(auc_tmp, auroc.all$AUC)
          }
