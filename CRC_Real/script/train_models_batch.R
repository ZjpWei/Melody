...
#                                                #
  ...
  # save model
  save(BW.prop.model, file = paste0(data.loc, "Models_batch/BW.prop.model.", s, ".Rdata"))
  
  # ################################# ALDEx2 ##################################
  # functions for ALDEx2
  source("./CRC_Real/script/aldex2.R")
  ...
  # save model
  save(Aldex2.model, file = paste0(data.loc, "Models_batch/Aldex2.model.", s, ".Rdata"))
  
  # ################################# ANCOM ###################################
  # functions for ANCOM-BC
  source("./CRC_Real/script/ancombc.R")
  # ANCOM-BC model
  ANCOMBC.model <- ancombc.fun(feature.table = feature.table, meta = meta.data,  formula = "labels + study")
  
  # save model
  save(ANCOMBC.model, file = paste0(data.loc, "Models_batch/ANCOMBC.model.", s, ".Rdata"))
  ...
  # save model
  save(clrlasso, file = paste0(data.loc, "Models_batch/CLR.lasso.model.", s,".Rdata"))
